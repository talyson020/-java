...
public class Carometro extends JFrame {
    ...
    public Carometro() {
        ...
        lblData.setBounds(10, 26, 400, 30);
        ...
        painelAlergias.setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED), "Alergias", TitledBorder.LEFT, TitledBorder.TOP, new Font("Tahoma", Font.BOLD, 11), Color.RED.darker()));
        ...
        painelObs.setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED), "Obs. Pedagógicas", TitledBorder.LEFT, TitledBorder.TOP, new Font("Tahoma", Font.BOLD, 11), new Color(0, 100, 0)));
        ...
        btnPdf.setBounds(580, 355, 64, 64);
    ...
    private void carregarFoto() {
        ...
        if (jfc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                Image foto = ImageIO.read(arquivo).getScaledInstance(lblFoto.getWidth(), lblFoto.getHeight(), Image.SCALE_SMOOTH);
    ...
    private void buscarRA() {
        try (Connection c = dao.conectar(); PreparedStatement p = c.prepareStatement("SELECT * FROM alunos WHERE ra = ?")) {
            p.setString(1, txtRA.getText());
            rs = p.executeQuery();
            if (rs.next()) preencherCampos();
            else { JOptionPane.showMessageDialog(null, "RA inexistente"); btnAdicionar.setEnabled(true); btnCarregar.setEnabled(true); }
        } catch (Exception e) { System.out.println(e); }
    }
    ...
    private void preencherCampos() throws Exception {
        ...
        lblFoto.setIcon(new ImageIcon(ImageIO.read(new ByteArrayInputStream(img)).getScaledInstance(lblFoto.getWidth(), lblFoto.getHeight(), Image.SCALE_SMOOTH)));
        ...
        btnEditar.setEnabled(true); btnExcluir.setEnabled(true); btnCarregar.setEnabled(true);
    }
    ...
    private void listarNomes() {
        ...
        try (Connection c = dao.conectar(); PreparedStatement p = c.prepareStatement("SELECT nome FROM alunos WHERE nome LIKE ?")) {
            p.setString(1, txtNome.getText() + "%");
            rs = p.executeQuery();
    ...
    private void buscarNome() {
        try (Connection c = dao.conectar(); PreparedStatement p = c.prepareStatement("SELECT * FROM alunos WHERE nome = ?")) {
            p.setString(1, listNomes.getSelectedValue());
            rs = p.executeQuery();
            if (rs.next()) { scrollPaneLista.setVisible(false); txtRA.setText(rs.getString("ra")); preencherCampos(); }
        } catch (Exception e) { System.out.println(e); }
    }

    private void editar() {
        String sql = fotoByte != null ? "UPDATE alunos SET nome=?, foto=?, alergias=?, observacoes=?, telefone=?, email=? WHERE ra=?" 
                                     : "UPDATE alunos SET nome=?, alergias=?, observacoes=?, telefone=?, email=? WHERE ra=?";
        try (Connection c = dao.conectar(); PreparedStatement p = c.prepareStatement(sql)) {
            if (fotoByte != null) {
                p.setString(1, txtNome.getText()); p.setBytes(2, fotoByte); p.setString(3, txtAlergias.getText());
                p.setString(4, txtObservacoes.getText()); p.setString(5, txtTelefone.getText()); p.setString(6, txtEmail.getText()); p.setString(7, txtRA.getText());
            } else {
                p.setString(1, txtNome.getText()); p.setString(2, txtAlergias.getText()); p.setString(3, txtObservacoes.getText());
                p.setString(4, txtTelefone.getText()); p.setString(5, txtEmail.getText()); p.setString(6, txtRA.getText());
            }
            p.executeUpdate(); JOptionPane.showMessageDialog(null, "Alterado!"); reset();
        } catch (Exception e) { System.out.println(e); }
    }

    private void excluir() {
        if (JOptionPane.showConfirmDialog(null, "Excluir?", "Aviso", JOptionPane.YES_NO_OPTION) == 0) {
            try (Connection c = dao.conectar(); PreparedStatement p = c.prepareStatement("DELETE FROM alunos WHERE ra=?")) {
                p.setString(1, txtRA.getText()); p.executeUpdate(); reset();
            } catch (Exception e) { System.out.println(e); }
        }
    }
    ...
    private void gerarPdf() {
        ...
        if (jfc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                tit.setAlignment(Element.ALIGN_CENTER); document.add(tit); document.add(new Paragraph(" "));
                
                // Redistribuição de pesos nas colunas
                PdfPTable tab = new PdfPTable(6); tab.setWidthPercentage(100);
                tab.setWidths(new float[]{4, 13, 15, 30, 24, 14}); // Foco total em Obs. Pedagógicas
                
                for (String h : new String[]{"RA", "Nome", "Alergias", "Obs. Pedagógicas", "Contato", "Foto"}) {
                    PdfPCell cell = new PdfPCell(new Phrase(h, fCab));
                    cell.setBackgroundColor(new BaseColor(30, 144, 255)); cell.setHorizontalAlignment(Element.ALIGN_CENTER); 
                    cell.setVerticalAlignment(Element.ALIGN_MIDDLE); cell.setPadding(6); tab.addCell(cell);
                }

                try (Connection c = dao.conectar(); ResultSet res = c.prepareStatement("SELECT * FROM alunos").executeQuery()) {
                    while (res.next()) {
                        ...
                        if(al != null && !al.isBlank() && !al.equalsIgnoreCase("Nenhuma")) cA.setBackgroundColor(new BaseColor(255, 210, 210));
                        tab.addCell(cA); 
                        
                        // OBS. PEDAGÓGICAS (Ajustado para textos longos)
                        String obs = res.getString("observacoes");
                        PdfPCell cObs = new PdfPCell(new Phrase(obs == null || obs.isBlank() ? "-" : obs, fNormal));
                        cObs.setPadding(6); // Espaçamento interno para legibilidade
                        tab.addCell(cObs);
                        ...
                        // Foto
                        Blob bl = res.getBlob("foto"); byte[] bytes = bl.getBytes(1, (int) bl.length());
                        com.itextpdf.text.Image im = com.itextpdf.text.Image.getInstance(bytes); im.scaleToFit(85, 85);
                        PdfPCell cFoto = new PdfPCell(im, false);
                        cFoto.setHorizontalAlignment(Element.ALIGN_CENTER); cFoto.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        cFoto.setPadding(4); tab.addCell(cFoto);
                    }
                }
                document.add(tab);
                JOptionPane.showMessageDialog(null, "PDF gerado!");
            } catch (Exception e) { System.out.println(e); } finally { document.close(); }
            try { Desktop.getDesktop().open(new File(caminho)); } catch (Exception e) { System.out.println(e); }
        }
    }

    private void reset() {
        txtRA.setText(null); txtNome.setText(null); txtTelefone.setText(null); txtEmail.setText(null);
        txtAlergias.setText(null); txtObservacoes.setText(null);
        lblFoto.setIcon(new ImageIcon(Carometro.class.getResource("/img/camera.png")));
        txtRA.setEditable(true); btnBuscar.setEnabled(true); btnAdicionar.setEnabled(false);
        btnEditar.setEnabled(false); btnExcluir.setEnabled(false); btnCarregar.setEnabled(false);
        fotoByte = null; scrollPaneLista.setVisible(false);
    }

    private void adicionarUndo(JTextComponent c) {
        UndoManager u = new UndoManager(); c.getDocument().addUndoableEditListener(e -> u.addEdit(e.getEdit()));
        c.addKeyListener(new KeyAdapter() { @Override public void keyPressed(KeyEvent e) {
            if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_Z && u.canUndo()) u.undo();
        }});
    }

    class TelefoneMask extends PlainDocument {
        @Override public void insertString(int o, String s, AttributeSet a) throws BadLocationException {
            if (s == null) return;
            String n = (getText(0, getLength()) + s).replaceAll("[^0-9]", "");
            if (n.length() > 11) return;
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < n.length(); i++) {
                if (i == 0) sb.append("("); else if (i == 2) sb.append(") "); else if (i == 7) sb.append("-");
                sb.append(n.charAt(i));
            }
            super.remove(0, getLength()); super.insertString(0, sb.toString(), a);
        }
    }
}